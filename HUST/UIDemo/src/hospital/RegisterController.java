...
public class RegisterController {
    ...
    @FXML
    void initialize() {
        welcomeLabel.setText( patientName + "你好\t当前账户余额:" + patientYCYE + "元");
    ...
    private void setInputNameRegister() {
        ...
        if (pinyin.isEmpty()) {
            inputNameRegister.getItems().clear();
            inputNameRegister.getItems().addAll(DBConnector.getInstance().getRegisterInfo(departs[0], inputIfSp.getEditor().getText(), null));
        } else {
            pinyin = pinyin.toLowerCase();
            inputNameRegister.getItems().clear();
            inputNameRegister.getItems().addAll(DBConnector.getInstance().getRegisterInfo(departs[0], inputIfSp.getEditor().getText(), pinyin));
        }
    }
